...
#define DT_DRV_COMPAT snps_dwc_ether_xgmac

#include <zephyr/logging/log.h>
LOG_MODULE_REGISTER(eth_xgmac, CONFIG_ETHERNET_LOG_LEVEL);

#include "eth_xgmac_priv.h"
#include "eth_xgmac_reg.h"

static inline bool eth_xgmac_vlan_check_index(const struct device *dev, uint8_t index,
					      bool *ext_filter)
{
...
int eth_xgmac_vlan_set_filter(const struct device *dev, uint8_t index,
			      const struct eth_xgmac_vlan_filter *filter)
{
	const struct eth_xgmac_config *cfg = dev->config;
	bool ext_filter = false;

	if (!eth_xgmac_vlan_check_index(dev, index, &ext_filter)) {
		return -EINVAL;
	}
...
static inline bool eth_xgmac_mac_check_index(const struct device *dev, uint8_t index)
{
	const struct eth_xgmac_config *cfg = dev->config;
	uint32_t max_index =
...
int eth_xgmac_mac_set_filter(const struct device *dev, uint8_t index,
			     const struct eth_xgmac_mac_filter *filter)
{
	const struct eth_xgmac_config *cfg = dev->config;
	uint32_t reg_val;

	if (!eth_xgmac_mac_check_index(dev, index)) {
		return -EINVAL;
	}

	reg_val = (filter->addr[5] << 8) | filter->addr[4];
	sys_write32(reg_val | (filter->enable ? MAC_ADDRESSX_HIGH_AE : 0) |
			    (filter->set_dma_ch ? FIELD_PREP(MAC_ADDRESSX_HIGH_DCS, filter->dma_ch)
						: 0),
		    cfg->base + MAC_ADDRESSX_HIGH(index));
	reg_val = (filter->addr[3] << 24) | (filter->addr[2] << 16) | (filter->addr[1] << 8) |
		  filter->addr[0];
	sys_write32(reg_val, cfg->base + MAC_ADDRESSX_LOW(index));

	/* Programm indirect dma sel register if duplication is enabled*/
	if (cfg->da_duplication) {
		sys_write32(filter->dma_ch, cfg->base + MAC_INDIR_ACCESS_DATA);
...
int eth_xgmac_pps_set_irq_target_time(const struct device *dev, struct net_ptp_time *ts)
{
	const struct eth_xgmac_config *cfg = dev->config;
...
void eth_xgmac_pps_set_callback(const struct device *dev, void (*callback)(), void *callback_data)
{
...
void eth_xgmac_pps_enable_flexible(const struct device *dev)
{
	const struct eth_xgmac_config *cfg = dev->config;
...
int eth_xgmac_pps_start_single_pulse(const struct device *dev, struct net_ptp_time *ts,
				     uint32_t width)
{
	const struct eth_xgmac_config *cfg = dev->config;
...
uint32_t ts_check;
int eth_xgmac_pps_single_pulse_irq(const struct device *dev, struct net_ptp_time *ts,
				   uint32_t width)
{
	const struct eth_xgmac_config *cfg = dev->config;
...
static void eth_xgmac_dma_init(const struct device *dev)
{
	const struct eth_xgmac_config *cfg = dev->config;
	uint8_t ch;

	if (!cfg->dma_only) {
		eth_xgmac_dma_set_sysbus(dev);
	}

	for (ch = 0; ch < cfg->dma_tx_channel; ch++) {
		eth_xgmac_dma_tx_init(dev, ch);
		eth_xgmac_dma_tx_irq_enable(dev, ch);
	}
	for (ch = 0; ch < cfg->dma_rx_channel; ch++) {
		eth_xgmac_dma_rx_init(dev, ch);
		eth_xgmac_dma_rx_irq_enable(dev, ch);
	}
}

static void eth_xgmac_dma_rx_refill_work(struct k_work *work);

static inline void eth_xgmac_dma_rx_data_init(const struct device *dev, uint8_t dma_ch)
{
	const struct eth_xgmac_config *cfg = dev->config;
	struct eth_xgmac_data *data = dev->data;
	const struct eth_xgmac_dma_ch_config *dma_cfg = &cfg->dma_rx[dma_ch];
	struct eth_xgmac_dma_rx_ch_data *dma_data = &data->dma_rx[dma_ch];
	...
	k_work_init_delayable(&dma_data->refill_work, eth_xgmac_dma_rx_refill_work);
}

static void eth_xgmac_dma_rx_fill_desc(const struct device *dev, uint8_t dma_ch)
{
	const struct eth_xgmac_config *cfg = dev->config;
	struct eth_xgmac_data *data = dev->data;
	const struct eth_xgmac_dma_ch_config *dma_cfg = &cfg->dma_rx[dma_ch];
	...
	for (descs = 0; descs < desc_free; descs++) {
		...
		__ASSERT(eth_xgmac_rdes_own((void *)&dma_cfg->descs[dma_data->tail]) == false,
		...
		eth_xgmac_rdes_rd_set((void *)&dma_cfg->descs[dma_data->tail], frag);
	...
	if (descs != 0) {
		eth_xgmac_dma_rx_set_tail(dev, dma_ch);
	}
...
static void eth_xgmac_dma_rx_refill_work(struct k_work *work)
{
	...
	struct eth_xgmac_dma_rx_ch_data *dma_data =
		CONTAINER_OF(dwork, struct eth_xgmac_dma_rx_ch_data, refill_work);

	eth_xgmac_dma_rx_fill_desc(dma_data->dev, dma_data->ch);
}

static void eth_xgmac_dma_rx_process(const struct device *dev, uint8_t dma_ch)
{
	const struct eth_xgmac_config *cfg = dev->config;
	struct eth_xgmac_data *data = dev->data;
	const struct eth_xgmac_dma_ch_config *dma_cfg = &cfg->dma_rx[dma_ch];
	...
	while (dma_data->head != dma_data->tail) {
		...
		eth_xgmac_dma_rx_clear_irq(dev, dma_ch);
		/* stop here if hardware still owns it */
		if (eth_xgmac_rdes_own(desc)) {
			break;
		}

		/* a packet's first descriptor: */
		if (eth_xgmac_rdes_first(desc)) {
		...
		if (eth_xgmac_rdes_desc_error(desc)) {
			net_buf_unref(frag);
			if (pkt) {
				net_pkt_unref(pkt);
				pkt = NULL;
		...
		if (eth_xgmac_rdes_context(desc)) {
			net_buf_unref(frag);
		} else {
			size_t add_len = eth_xgmac_rdes_last(desc)
					 ? eth_xgmac_rdes_packet_length(desc) - net_pkt_get_len(pkt)
		...
		if (eth_xgmac_rdes_context(desc) && eth_xgmac_rdes_ts_valid(desc)) {
			struct net_ptp_time ts;
			eth_xgmac_rdes_get_ts(desc, &ts);
			ts._sec.high = eth_xgmac_ts_get_high(dev);
			net_pkt_set_timestamp(pkt, &ts);
		}
#endif

#if defined(CONFIG_NET_VLAN)
		if (eth_xgmac_rdes_last(desc) && eth_xgmac_rdes_vlan_valid(desc)) {
			net_pkt_set_vlan_tci(pkt, eth_xgmac_rdes_get_tci(desc));
			net_pkt_set_priority(pkt, net_vlan2priority(net_pkt_vlan_priority(pkt)));
		}
#endif

		/* Packet reception error */
		if (eth_xgmac_rdes_pkt_error(desc)) {
			eth_stats_update_errors_rx(data->iface);
			net_pkt_unref(pkt);
			pkt = NULL;
			goto next;
		...
		if (eth_xgmac_rdes_pkt_finish(desc)) {
...
static void eth_xgmac_dma_tx_process(const struct device *dev, uint8_t dma_ch)
{
	const struct eth_xgmac_config *cfg = dev->config;
	struct eth_xgmac_data *data = dev->data;
	struct net_buf *frag;
	const struct eth_xgmac_dma_ch_config *dma_cfg = &cfg->dma_tx[dma_ch];
	struct eth_xgmac_dma_tx_ch_data *dma_data = &data->dma_tx[dma_ch];

	while (dma_data->head != dma_data->tail) {
		eth_xgmac_dma_tx_clear_irq(dev, dma_ch);
		/* stop here if hardware still owns it */
		if (eth_xgmac_tdes_own((void *)&dma_cfg->descs[dma_data->head])) {
			break;
		}
		...
		if (eth_xgmac_tdes_last((void *)&dma_cfg->descs[dma_data->head])) {
			/* log any errors */
			if (eth_xgmac_tdes_error((void *)&dma_cfg->descs[dma_data->head])) {
				...
				eth_xgmac_dma_ts_handle(dev, dma_ch);
			}
#endif
		}
...
static inline void eth_xgmac_dma_tx_data_init(const struct device *dev, uint8_t dma_ch)
{
	const struct eth_xgmac_config *cfg = dev->config;
	const struct eth_xgmac_dma_ch_config *dma_cfg = &cfg->dma_tx[dma_ch];
	struct eth_xgmac_data *data = dev->data;
...
static inline void eth_xgmac_mac_init(const struct device *dev)
{
	eth_xgmac_mac_config(dev);
}

static inline void eth_xgmac_mtl_init(const struct device *dev)
{
	const struct eth_xgmac_config *cfg = dev->config;
	uint32_t queue;

	for (queue = 0; queue < cfg->mtl_rx_queues; queue++) {
		eth_xgmac_mtl_rxq_init(dev, queue);
	}

	for (queue = 0; queue < cfg->mtl_tx_queues; queue++) {
		eth_xgmac_mtl_txq_init(dev, queue);
	}

	eth_xgmac_mtl_set_rxq_ctrl(dev);

	/* set up MTL */
	eth_xgmac_mtl_set_mode(dev, 0);
}

static void eth_xgmac_set_link_state(const struct device *dev, struct phy_link_state *link_state)
{
	const struct eth_xgmac_config *cfg = dev->config;
	uint8_t dma_ch;
	uint8_t ss = eth_xgmac_mac_get_speed(link_state->speed);
	...
	if (eth_xgmac_mac_rx_state(dev) == link_state->is_up ||
	    eth_xgmac_mac_tx_state(dev) == link_state->is_up) {
		arch_irq_unlock(key);
		return;
	}
	...
	if (link_state->is_up) {
		...
		eth_xgmac_mac_set_link(dev, ss, fd);
	}
	/* Operate TX DMA if clock doesn't stop */
	if (!cfg->clocks_stop) {
		if (link_state->is_up) {
			for (dma_ch = 0; dma_ch < cfg->dma_tx_channel; dma_ch++) {
				eth_xgmac_dma_tx_start(dev, dma_ch);
			}
		} else {
			/* Disable DMA*/
			for (dma_ch = 0; dma_ch < cfg->dma_tx_channel; dma_ch++) {
				eth_xgmac_dma_tx_stop(dev, dma_ch);
			}
			/*  Wait for queue emtpy or flush */
			for (dma_ch = 0; dma_ch < cfg->dma_tx_channel; dma_ch++) {
				eth_xgmac_dma_tx_queue_flush(dev, dma_ch);
			}
		}
	}
	/* Disable or enable receiver*/
	eth_xgmac_mac_rx_set_state(dev, link_state->is_up);
	/* Disable or enable transmitter and set mac configuration */
	eth_xgmac_mac_tx_set_state(dev, link_state->is_up);
	arch_irq_unlock(key);
}

static void eth_xgmac_link_state_changed(const struct device *phy_dev,
					 struct phy_link_state *link_state, void *user_data)
{
	...
	if (link_state->is_up) {
		if (cfg->clocks_stop) {
			eth_xgmac_mac_wait_idle(dev);
		} else {
		}
		eth_xgmac_set_link_state(dev, link_state);
...
void eth_xgmac_iface_init(struct net_if *iface)
{
	...
	if (!cfg->dma_only) {
		struct eth_xgmac_mac_filter filter = {.addr = {0},
		...
		eth_xgmac_mac_set_filter(dev, 0, &filter);
	}
	...
	if (cfg->phy) {
		...
		phy_link_callback_set(cfg->phy, &eth_xgmac_link_state_changed, (void *)dev);
	} else {
		net_if_carrier_on(iface);
	}
}

int eth_xgmac_start(const struct device *dev)
{
	const struct eth_xgmac_config *cfg = dev->config;
	struct eth_xgmac_data *data = dev->data;
	...
	for (dma_ch = 0; dma_ch < cfg->dma_rx_channel; dma_ch++) {
		eth_xgmac_dma_rx_fill_desc(dev, dma_ch);
		eth_xgmac_dma_rx_start(dev, dma_ch);
	}
	...
	if (cfg->phy) {
		...
		if (state.is_up) {
			net_if_carrier_on(data->iface);
			eth_xgmac_set_link_state(dev, &state);
		}
	}
...
static int eth_xgmac_send(const struct device *dev, struct net_pkt *pkt)
{
	const struct eth_xgmac_config *cfg = dev->config;
	struct eth_xgmac_data *data = dev->data;
	uint8_t dma_ch = 0;
	...
	if (net_pkt_is_tx_timestamping(pkt) || net_pkt_is_ptp(pkt)) {
		...
		if (eth_xgmac_mac_reserve_tx_ts(dev, pkt, K_MSEC(1000)) != 0) {
	...
	while (frag != NULL) {
		...
		eth_xgmac_tdes_rd_set_frag(tdes, frag);
		eth_xgmac_tdes_rd_set(
	...
	/* Notify the hardware */
	eth_xgmac_dma_tx_set_tail(dev, dma_ch);
...
#if CONFIG_PTP_CLOCK
const struct device *eth_xgmac_get_ptp_clock(const struct device *dev)
{
	const struct eth_xgmac_config *cfg = dev->config;
...
static const struct device *eth_xgmac_get_phy(const struct device *dev)
{
	return ((const struct eth_xgmac_config *)dev->config)->phy;
}

void eth_xgmac_common_isr(const struct device *dev)
{
	const struct eth_xgmac_config *cfg = dev->config;
	__maybe_unused struct eth_xgmac_data *data = dev->data;
	...
	if (mac_status & MAC_INTERRUPT_STATUS_TSIS) {
...
#endif
			if (ts_status & MAC_TIMESTAMP_STATUS_TXTSC) {
#if defined(CONFIG_NET_PKT_TIMESTAMP)
				uint32_t i;
				for (i = 0; i < FIELD_GET(MAC_TIMESTAMP_STATUS_TTSNS, ts_status);
				     i++) {
					struct net_ptp_time ts;
					struct net_pkt *pkt =
						eth_xgmac_net_pkt_slist_get(&data->ptp_pkts);
					__ASSERT_NO_MSG(pkt);

					k_sem_give(&data->ptp_pkt_sem);

					ts.nanosecond = sys_read32(
						cfg->base + MAC_TX_TIMESTAMP_STATUS_NANOSECONDS);
					ts._sec.low = sys_read32(cfg->base +
								 MAC_TX_TIMESTAMP_STATUS_SECONDS);
					ts._sec.high = sys_read32(
						cfg->base + MAC_SYSTEM_TIME_HIGHER_WORD_SECONDS);
					ts._sec.unused = 0;
					net_pkt_set_timestamp(pkt, &ts);
		...
		} while (ts_status & MAC_TIMESTAMP_STATUS_TXTSC);
	}
	if (dma_status) {
		uint8_t dma_ch;
		...
		for (dma_ch = 0; dma_ch < cfg->dma_tx_channel; dma_ch++) {
			...
			if (dma_ch_status & DMA_CHi_STATUS_TI) {
				eth_xgmac_dma_tx_process(dev, dma_ch);
			}
		#endif
			sys_write32(DMA_CHi_STATUS_NIS | DMA_CHi_STATUS_AIS | DMA_CHi_STATUS_TBU |
					    DMA_CHi_STATUS_FBE | DMA_CHi_STATUS_CDE,
		...
		for (dma_ch = 0; dma_ch < cfg->dma_rx_channel; dma_ch++) {
			...
			if (dma_ch_status & (DMA_CHi_STATUS_RI | DMA_CHi_STATUS_RBU)) {
				/* Process descriptors */
				eth_xgmac_dma_rx_process(dev, dma_ch);
				/* Fill up descriptors again */
				eth_xgmac_dma_rx_fill_desc(dev, dma_ch);
			}
#endif
			sys_write32(DMA_CHi_STATUS_NIS | DMA_CHi_STATUS_AIS | DMA_CHi_STATUS_RBU |
					    DMA_CHi_STATUS_DDE | DMA_CHi_STATUS_FBE |
					    DMA_CHi_STATUS_CDE,
...
#define TX_IRQ(n, _)                                                                               \
	__unused static void eth_xgmac_tx_ch##n##_isr(const struct device *dev)                    \
	{                                                                                          \
		eth_xgmac_dma_tx_process(dev, n);                                                  \
	}
LISTIFY(8, TX_IRQ, ())

#define RX_IRQ(n, _)                                                                               \
	__unused static void eth_xgmac_rx_ch##n##_isr(const struct device *dev)                    \
	{                                                                                          \
		/* Process descriptors */                                                          \
		eth_xgmac_dma_rx_process(dev, n);                                                  \
		/* Fill up descriptors again */                                                    \
		eth_xgmac_dma_rx_fill_desc(dev, n);                                                \
	}
LISTIFY(8, RX_IRQ, ())

#if defined(CONFIG_NET_STATISTICS_ETHERNET)
struct net_stats_eth *eth_xgmac_get_stats(const struct device *dev)
{
	ARG_UNUSED(dev);
	return NULL;
}
...
int eth_xgmac_stop(const struct device *dev)
{
	return 0;
}

enum ethernet_hw_caps eth_xgmac_get_capabilities(const struct device *dev)
{
	const struct eth_xgmac_config *cfg = dev->config;
	...
	/* Default speed */
	caps |= ETHERNET_LINK_2500BASE | ETHERNET_LINK_5000BASE;
	/* Default settings */
	caps |= ETHERNET_HW_VLAN | ETHERNET_PROMISC_MODE;

	/* Check for slow link capabilites */
	if ((hw_feature0 & MAC_HW_FEATURE0_GMIISEL) || (hw_feature0 & MAC_HW_FEATURE0_RGMIISEL) ||
	    (hw_feature0 & MAC_HW_FEATURE0_RMIISEL)) {
...
int eth_xgmac_set_config(const struct device *dev, enum ethernet_config_type type,
...
int eth_xgmac_get_config(const struct device *dev, enum ethernet_config_type type,
...
#if defined(CONFIG_NET_VLAN)
int eth_xgmac_vlan_setup(const struct device *dev, struct net_if *iface, uint16_t tag, bool enable)
{
	return 0;
}
#endif /* CONFIG_NET_VLAN */

int eth_xgmac_init(const struct device *dev)
{
	const struct eth_xgmac_config *cfg = dev->config;
	struct eth_xgmac_data *data = dev->data;
	uint32_t dma_ch;
	...
	if (!WAIT_FOR((sys_read32(cfg->DMA_BASE + DMA_MODE) & DMA_MODE_SWR) == 0, 2500000,
		      k_busy_wait(10))) {
		LOG_ERR("MAC reset failed");
		return -EIO;
	}
	...
	for (dma_ch = 0; dma_ch < cfg->dma_tx_channel; dma_ch++) {
		eth_xgmac_dma_tx_data_init(dev, dma_ch);
	}

	for (dma_ch = 0; dma_ch < cfg->dma_rx_channel; dma_ch++) {
		eth_xgmac_dma_rx_data_init(dev, dma_ch);
	}

#if IS_ENABLED(CONFIG_PTP_CLOCK)
	uint32_t count =
		(1 << (FIELD_GET(MAC_HW_FEATURE3_TTSFD, sys_read32(cfg->base + MAC_HW_FEATURE3)) -
		       1));
	k_sem_init(&data->ptp_pkt_sem, count, count);
	sys_slist_init(&data->ptp_pkts);
#endif
	...
	if (!cfg->dma_only) {
		...
		eth_xgmac_mac_init(dev);
		eth_xgmac_mtl_init(dev);
	}
	eth_xgmac_dma_init(dev);

	return 0;
}

struct ethernet_api eth_xgmac_api = {
	.iface_api.init = eth_xgmac_iface_init,
	.start = eth_xgmac_start,
	.stop = eth_xgmac_stop,
	.get_config = eth_xgmac_get_config,
	.get_capabilities = eth_xgmac_get_capabilities,
	.set_config = eth_xgmac_set_config,
#if defined(CONFIG_NET_VLAN)
	.vlan_setup = eth_xgmac_vlan_setup,
#endif
#if defined(CONFIG_PTP_CLOCK)
	.get_ptp_clock = eth_xgmac_get_ptp_clock,
#endif
	.get_phy = eth_xgmac_get_phy,
	.send = eth_xgmac_send,
};

#define ETH_XGMAC_INIT(n)                                                                          \
	ETH_XGMAC_INIT_FUNC(n)                                                                     \
	ETH_XGMAC_DMA_INIT(n);                                                                     \
	ETH_XGMAC_MTL_INIT(n);                                                                     \
	static struct eth_xgmac_data eth_xgmac_data_##n = ETH_XGMAC_DATA(n);                       \
	static struct eth_xgmac_config eth_xgmac_config_##n = ETH_XGMAC_CONFIG(n);                 \
                                                                                                   \
	ETH_NET_DEVICE_DT_INST_DEFINE(n, eth_xgmac_init, NULL, &eth_xgmac_data_##n,                \
				      &eth_xgmac_config_##n, CONFIG_ETH_INIT_PRIORITY,             \
				      &eth_xgmac_api, NET_ETH_MTU);

DT_INST_FOREACH_STATUS_OKAY(ETH_XGMAC_INIT)
