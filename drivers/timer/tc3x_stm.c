...
{
	uint64_t time = sys_read32(TIMER_BASE_ADDR + offsetof(Ifx_STM, TIM0SV));
	time |= ((uint64_t)sys_read32(TIMER_BASE_ADDR + offsetof(Ifx_STM, TIM0SV) + 4) << 32);
	return time;
}

__maybe_unused static uint32_t get_compare()
{
	return sys_read32(TIMER_BASE_ADDR + offsetof(Ifx_STM, CMP));
}
