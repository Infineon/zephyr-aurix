...
static inline int clock_control_tc3x_ccu_osc_init(void)
{
	...
	if (!WAIT_FOR(MODULE_SCU.OSCCON.B.PLLHV == 1 && MODULE_SCU.OSCCON.B.PLLLV == 1, 10000,
		      ccu_busy_wait_fback(1))) {
		return -EIO;
	}
...
static inline int clock_control_tc3x_ccu_pll_init(void)
{
	...
	if (!WAIT_FOR(MODULE_SCU.PERPLLSTAT.B.PWDSTAT == 0 && MODULE_SCU.SYSPLLSTAT.B.PWDSTAT == 0,
...
static inline int clock_control_tc3x_ccu_pll_power_down(void)
{
	...
	if (!WAIT_FOR(MODULE_SCU.PERPLLSTAT.B.PWDSTAT == 1 && MODULE_SCU.SYSPLLSTAT.B.PWDSTAT == 1,
...
static inline int clock_control_tc3x_ccu_pll_set_devider(void)
{
	...
	uint32_t current_div = syspllcon1.B.K2DIV + 1;
	bool move_up = current_div < PLL_K_DIV(sys_pll, 2);

	while (current_div != PLL_K_DIV(sys_pll, 2)) {
		current_div += move_up ? 1 : -1;
		syspllcon1.B.K2DIV = current_div - 1;
...
static inline int clock_control_tc3x_ccu_pll_initial_divider(void)
{
	...
	if (!WAIT_FOR(MODULE_SCU.SYSPLLSTAT.B.K2RDY == 1 && MODULE_SCU.PERPLLSTAT.B.K2RDY == 1 &&
		      MODULE_SCU.PERPLLSTAT.B.K3RDY == 1,
		      1000, k_busy_wait(1))) {
		return -EIO;
	}
	return 0;
...
static int clock_control_tc3x_ccu_on(const struct device *dev, clock_control_subsys_t sys)
{
	...
	case CLOCK_FQSPI:
		return CLOCK_DIV_WITH_INST(fqspi, infineon_aurix_qspi) != 0 ? 0 : -EIO;
	case CLOCK_FADC:
		return 0;
	case CLOCK_FI2C:
		return CLOCK_DIV_WITH_INST(fi2c, infineon_aurix_i2c) != 0 ? 0 : -EIO;
	case CLOCK_FEBU:
		return 0;
	default:
		return -EINVAL;
					   ...
					   uint32_t *rate)
{
	...
	case CLOCK_FASCLINS:
		*rate = CLOCK_SOURCE_IS(fasclins, fasclinsi) ? fsource1_freq / fasclinsi_div
		...
		return 0;
	case CLOCK_FQSPI:
		if (CLOCK_DIV_WITH_INST(fqspi, infineon_aurix_qspi) == 0) {
			return -EIO;
		}
...
static int clock_control_tc3x_ccu_init(const struct device *dev)
{
	...
	/* Only the master core (cpu0) configures the CCU; slave cores skip
	 * init to avoid double-programming PLLs and clocks.
	 */
	...
	if (CLOCK_IS_CLOCK_SOURCE(fpll0, fsource0)) {
		...
		ret = clock_control_tc3x_ccu_pll_init();
		if (ret) {
			return ret;
		}

		/* Wait for 1 ms to stabilize the plls */
		ccu_busy_wait_fback(1000);

		if (MODULE_SCU.PERPLLSTAT.B.LOCK != 1 || MODULE_SCU.SYSPLLSTAT.B.LOCK != 1) {
			return -EIO;
		}
	...
	const bool fsource0_is_fpll0 =
		DT_SAME_NODE(DT_CLOCKS_CTLR(DT_NODELABEL(fsource0)), DT_NODELABEL(fpll0));

	ret = clock_control_tc3x_ccu_select_clock(fsource0_is_fpll0);
	if (ret) {
		return ret;
	}
...
static DEVICE_API(clock_control, clock_control_tc3x_ccu_api) = {
