...
static inline int clock_control_tc3x_ccu_osc_init()
{
...
static inline void clock_control_tc3x_ccu_osc_disable()
{
	MODULE_SCU.OSCCON.B.MODE = 1;
}

static inline bool clock_control_tc3x_ccu_osc_ok()
{
	Ifx_SCU_OSCCON osccon = MODULE_SCU.OSCCON;
...
static inline void clock_control_tc3x_ccu_pll_init()
{
...
static inline int clock_control_tc3x_ccu_pll_power_down()
{
...
static inline int clock_control_tc3x_ccu_pll_set_devider()
{
...
static inline int clock_control_tc3x_ccu_pll_wait_lock()
{
	Ifx_SCU_PERPLLSTAT perpllstat;
	Ifx_SCU_SYSPLLSTAT syspllstat;
...
static inline int clock_control_tc3x_ccu_pll_initial_divider()
{
...
static inline int clock_control_tc3x_ccu_set_divider()
{
...
static int clock_control_tc3x_ccu_on(const struct device *dev, clock_control_subsys_t sys)
{
	...
	case CLOCK_FADAS:
		return 0;
	case CLOCK_FMCANH:
		/* fmcanh is shared by GETH and MCMCAN; either consumer brings
		 * the divider out of zero.  CLOCK_DIV_WITH_INST goes through
		 * any node with the matching compat, so either path is fine.
		 */
