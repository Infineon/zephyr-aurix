...
static inline int clock_control_tc4x_ccu_osc_init()
{
...
static inline int clock_control_tc4x_ccu_syspll_default()
{
...
static inline int clock_control_tc4x_ccu_syspll_init()
{
...
static inline int clock_control_tc4x_ccu_syspll_divider()
{
	Ifx_CLOCK_SYSPLLCON1 con1 = CLOCK_SYSPLLCON1;
...
static inline int clock_control_tc4x_ccu_perpll_default()
{
...
static inline int clock_control_tc4x_ccu_perpll_init()
{
...
static inline int clock_control_tc4x_ccu_perpll_divider()
{
	Ifx_CLOCK_PERPLLCON1 con1 = CLOCK_PERPLLCON1;
...
static inline int clock_control_tc4x_ccu_set_divider()
{
	...
	Ifx_CLOCK_PERCCUCON0 perccu0 = {
		/* TC4Dx silicon: PERCCUCON0.CLKSELMCAN must be 1 (fsource0) for the
		 * BSP to clock at the rate the bit-timing solver expects.  With
		 * CLKSELMCAN = 2 (fsource1, what the DT clock tree resolves to via
		 * fmcani -> fsource1) every TX hits StuffError on arbitration even
		 * though all M_CAN registers match a known-good NuttX baseline.
		 * NuttX inherits CLKSELMCAN = 1 from SSW and works.  Verified live
		 * on TC4D7 Lite 2026-05-01.
		 */
		.B.CLKSELMCAN = 1U,
