...
static int can_mcan_leave_init_mode(const struct device *dev, k_timeout_t timeout)
{
	...
	} while ((cccr & CAN_MCAN_CCCR_CCE) != 0U);
...
int can_mcan_init(const struct device *dev)
{
...

	{
		...
		err = can_mcan_read_reg(dev, CAN_MCAN_TEST, &test_reg);
		if (err != 0) {
			return err;
		}
		test_reg &= ~(CAN_MCAN_TEST_LBCK);
		err = can_mcan_write_reg(dev, CAN_MCAN_TEST, test_reg);
		if (err != 0) {
			return err;
		}
	}
