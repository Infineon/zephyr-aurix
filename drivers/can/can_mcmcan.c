...
static int mcmcan_mcan_get_core_clock(const struct device *dev, uint32_t *rate)
{
	...
	/* The BSP samples bits from the synchronous clock (fmcanh), not the
	 * asynchronous host-interface clock (fmcani at clock_id[0]). Reporting
	 * the wrong rate makes Zephyr's bit-timing solver scale BRP/TSEG by
	 * 80/100, so a 500 kbit/s setting drives the bus at 400 kbit/s.
	 * Loopback hides this since LBCK shortcuts the BSP.
	 */
	return clock_control_get_rate(mcmcan_config->clock_dev,
				      (clock_control_subsys_t)&mcmcan_config->clock_id[1], rate);
}
