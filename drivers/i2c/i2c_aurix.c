	...
	for (msg_idx = 0; msg_idx < num_msgs; msg_idx++) {
		...
		if (msgs[msg_idx].flags & I2C_MSG_STOP) {
			if (!(ev & BIT(0))) {
				 ...
				 * Do NOT clear irq_event BIT(0) first: on TC3x,
				 * NACK and TX_END (SONA auto-stop) fire as separate
				 * interrupts; the TX_END ISR may have already posted
				 * BIT(0) between the k_event_wait above returning and
				 * this point.  Clearing would race and cause a
				 * K_FOREVER stall on an already-idle bus. */
...
err:
	/* Stop bus if not stopped and error happened */
